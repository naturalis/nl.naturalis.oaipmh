...
public class RequestBuilder {
  ...
  /*
   * This currently just returns a new instance, but in the future we might want to retrieve configuration data just before creating
   * the first instance.
   */
  ...
  /**
   * Sets the resumption token parser used to decompose the resumptionToken query parameter (if any). You would ordinarily retrieve
   * the parser from an {@link IOAIRepository} implementation. If this method is not called before calling {@link #build(UriInfo)},
   * the {@link ResumptionToken default parser} is used.
   * 
   * @param parser
   * @return
   */
  ...
  /**
   * Creates an {@code OAIPMHRequest} instance from the specified {@link UriInfo} object.
   * 
   * @param uriInfo
   * @return
   */
  ...
  /*
   * Check for duplicate and illegal parameters. Returns all valid OAIPMH arguments in the request except the verb, which is more
   * like the name of the function to be executed.
   */
