...
public class XMLSerialisableRootElementFactory {
  ...
  /**
   * Converts the XML contents of the plugin_document_xml column to an instance of {@link XMLSerialisableRootElement}.
   * 
   * @param root
   * @return
   */
  ...
  private static List<String> getInputDocuments(Element root) {
    ...
    if (elems != null) {
      return elems.stream().map(e -> e.getAttribute("weakReference")).collect(Collectors.toList());
    }
    return null;
  }
