...
/**
 * A {@link IAnnotatedDocumentPreFilter pre-filter} for DNA extract plates. This filter filters out any record that does not
 * contain the string "&lt;ExtractPlateNumberCode_Samples&gt;" in its document_xml column.
 * 
...
public class DnaExtractPlatePreFilter implements IAnnotatedDocumentPreFilter {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DnaExtractPlatePreFilter.class);
  ...
  public boolean accept(ResultSet rs) throws SQLException {
    ...
    if (xml.indexOf("<ExtractPlateNumberCode_Samples>") == -1) {
      if (logger.isDebugEnabled()) {
        logger.debug("Record discarded. Missing required element <ExtractPlateNumberCode_Samples> in document_xml");
      }
      ++numDiscarded;
      return false;
    }
