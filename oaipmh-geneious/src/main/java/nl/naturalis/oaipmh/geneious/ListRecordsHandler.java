 ...
 * <p>
 * This describes how database records are filtered and sorted as part of the OAI-PMH generation process:
 * <ol>
 * <li>First, a SQL query is issued that only selects records from the annotated_document table whose reference_count column
 * equals 0. See {@link ReferenceComparator} for an explanantion. No ORDER BY clause is used; filters applied subsequently cannot
 * rely on the records being sorted in any particular order.
 * <li>Then the {@link SharedPreFilter} is applied. See {@link IAnnotatedDocumentPreFilter here} for an explanation of
 * pre-filters.
 * <li>Then all repository-specific pre-filters are applied. Any number of additional pre-filters can be defined for specimens,
 * DNA extracts and DNA extract plates.
 * <li>Then the {@link SharedPostFilter} is applied. See {@link IAnnotatedDocumentPostFilter here} for an explanation of
 * post-filters.
 * <li>Then all repository-specific post-filters are applied.
 * <li>Then the {@link DocumentVersionSetFilter} is applied. See {@link IAnnotatedDocumentSetFilter here} for an explanation of
 * set filters.
 * <li>Then all repository-specific set filters are applied.
 * <li>Then the remaining records are sorted in descending order of their database ID.
 * </ol>
 * </p>
 * 
...
public abstract class ListRecordsHandler {
  ...
  /**
   * Template method to be implemented by subclasses: provide extra pre-filters for the particular resource the subclass deals with.
   * The extra pre-filters are applied <i>after</i> the {@link SharedPreFilter}.
   */
  protected abstract List<IAnnotatedDocumentPreFilter> getAnnotatedDocumentPreFilters();

  /**
   * Template method to be implemented by subclasses: provide extra post-filters for the particular resource the subclass deals
   * with. The extra post-filters are applied <i>after</i> the {@link SharedPostFilter}.
   */
  protected abstract List<IAnnotatedDocumentPostFilter> getAnnotatedDocumentPostFilters();

  /**
   * Template method to be implemented by subclasses: provide extra set filters for the particular resource the subclass deals with.
   * The extra post-filters are applied <i>after</i> the {@link DocumentVersionSetFilter}.
   */
  protected abstract List<IAnnotatedDocumentSetFilter> getAnnotatedDocumentSetFilters();

  /**
   * Template method to be implemented by subclasses: provide processors for {@link AnnotatedDocument} instances once they have gone
   * through all filters.
   * 
   * @return
   */
  ...
  /**
   * Template method to be implemented by subclasses: provide the child element of the &lt;geneious&gt; element.
   * 
   * @param geneious
   * @param ad
   */
  ...
  /**
   * Template method to be implemented by concrete subclasses: return the page size (number of records per request) for the
   * particular resource to subclass deals with.
   * 
   * @return
   */
  ...
  /**
   * Returns the database query retrieving the initial set of records (before any programmatical filtering). Could be overrided by
   * subclasses should the need arise.
   * 
   * @return
   */
  protected String getSQLQuery() {
    ...
    if (request.getFrom() != null) {
      /*
       * Column "modified" contains the number of seconds since 01-01-1970 while Date.getTime() returns the number of milliseconds since
       * 01-01-1970.
       */
