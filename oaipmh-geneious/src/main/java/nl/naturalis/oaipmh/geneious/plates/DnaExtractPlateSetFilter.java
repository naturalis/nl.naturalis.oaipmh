...
/**
 * A {@link IAnnotatedDocumentSetFilter set-filter} for DNA extract plates. Ensures that for each extract plate only one record
 * from the annotated_document table is selected. This is, more or less arbitrarily, the record with the highest database ID. In
 * other words, for all records with the same {@link DocumentNotes.Note#ExtractPlateNumberCode_Samples plate number}), only the
 * one with the highest database ID is selected and turned into a &lt;DnaExtractPlate&gt; element in the OAI-PMH output.
 * 
...
public class DnaExtractPlateSetFilter implements IAnnotatedDocumentSetFilter {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DnaExtractPlateSetFilter.class);
  ...
  public List<AnnotatedDocument> filter(List<AnnotatedDocument> input) {
    if (logger.isDebugEnabled()) {
      ...
      logger.debug("Marking records for removal using {}", PlateNumberComparator.class.getSimpleName());
    }
