...
/**
 * A {@link IAnnotatedDocumentSetFilter set-filter} for Specimens. Ensures that for each specimen only one record from the
 * annotated_document table is selected. This is, more or less arbitrarily, the record with the highest database ID. In other
 * words, for all records with the same {@link DocumentNotes.Note#RegistrationNumberCode_Samples specimen registration number}),
 * only the one with the highest database ID is selected and turned into a &lt;Specimen&gt; element in the OAI-PMH output.
 * 
...
public class SpecimenSetFilter implements IAnnotatedDocumentSetFilter {
  ...
  private static final Logger logger = LoggerFactory.getLogger(SpecimenSetFilter.class);
  ...
  public List<AnnotatedDocument> filter(List<AnnotatedDocument> input) {
    if (logger.isDebugEnabled()) {
      ...
      logger.debug("Marking records for removal using {}", SpecimenUnitIDComparator.class.getSimpleName());
    }
