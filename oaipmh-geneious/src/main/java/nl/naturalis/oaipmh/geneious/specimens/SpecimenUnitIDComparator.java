...
/**
 * A {@link Comparator} for {@link AnnotatedDocument} instances that, per specimen, selects the instance with the highest database
 * ID.
 * 
...
public class SpecimenUnitIDComparator implements Comparator<AnnotatedDocument> {
  ...
  private static final Logger logger = LoggerFactory.getLogger(SpecimenUnitIDComparator.class);
  ...
  public int compare(AnnotatedDocument ad0, AnnotatedDocument ad1) {
    ...
    int i = cmp(RegistrationNumberCode_Samples, notes0, notes1);
    if (i != 0) {
      return i;
    }
    ...
    if (i < 0) {
      ...
      if (!ad0.doNotOutput && logger.isDebugEnabled()) {
        logger.debug(MSG1, ad0.getId(), RegistrationNumberCode_Samples, ad1.getId());
      }
      ad0.doNotOutput = true;
      return i;
    }
    if (!ad1.doNotOutput && logger.isDebugEnabled()) {
      logger.debug(MSG1, ad1.getId(), RegistrationNumberCode_Samples, ad0.getId());
    }
    ad1.doNotOutput = true;
    return i;
  }
