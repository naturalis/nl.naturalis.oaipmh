...
/**
 * Compares {@link AnnotatedDocument} instances based on whether one references the other. If one {@code AnnotatedDocument}
 * references another {@code AnnotatedDocument}, it means it has superseded the other {@code AnnotatedDocument}. Thus, the
 * referenced {@code AnnotatedDocument} should not end up in the OAI-PMH output. Filtering by means of this comparator can
 * optionally be enabled in the {@link DocumentVersionSetFilter}. However, this comparator can most likely be done away with (see
 * comments for {@link DocumentVersionSetFilter}).
 * 
 * @deprecated Using this comparator is expensive and just checking the reference count column of the annotated_document table
 *             seems to achieve exactly the same.
 * 
...
public class ReferenceComparator implements Comparator<AnnotatedDocument> {
  ...
  private static final Logger logger = LoggerFactory.getLogger(ReferenceComparator.class);
  ...
  /**
   * Compares the specified {@link AnnotatedDocument} instances. A side-effect of the comparison is that the referenced document
   * will be marked as dispensable. That is, it will be excluded from the OAI-PMH output.
   */
  @Override
  ...
  /**
   * Returns the number of dispensable records this comparator instance has been fed so far.
   * 
   * @return
   */
