...
/**
 * A {@link IAnnotatedDocumentSetFilter set-filter} for DNA extracts. Ensures that for each DNA extract only one record from the
 * annotated_document table is selected. This is, more or less arbitrarily, the record with the highest database ID. In other
 * words, for all records with the same {@link DocumentNotes.Note#ExtractIDCode_Samples extract ID}), only the one with the
 * highest database ID is selected and turned into a &lt;DnaExtract&gt; element in the OAI-PMH output.
 * 
...
public class DnaExtractSetFilter implements IAnnotatedDocumentSetFilter {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DnaExtractSetFilter.class);
  ...
  public List<AnnotatedDocument> filter(List<AnnotatedDocument> input) {
    if (logger.isDebugEnabled()) {
      ...
      logger.debug("Marking records for removal using {}", DnaExtractComparator.class.getSimpleName());
    }
