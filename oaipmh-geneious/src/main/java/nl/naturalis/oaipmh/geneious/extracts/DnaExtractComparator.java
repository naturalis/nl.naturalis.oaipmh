...
/**
 * A {@link Comparator} for {@link AnnotatedDocument} records used to select those on the basis of their DNA extract contents.
 * 
...
public class DnaExtractComparator implements Comparator<AnnotatedDocument> {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DnaExtractComparator.class);
  ...
  public int compare(AnnotatedDocument ad0, AnnotatedDocument ad1) {
    ...
    int i;
    if ((i = cmp(ExtractIDCode_Samples, notes0, notes1)) != 0) {
      return i;
    }

    if ((i = cmp(RegistrationNumberCode_Samples, notes0, notes1)) != 0) {
      return i;
    }

    if ((i = cmp(PlatePositionCode_Samples, notes0, notes1)) != 0) {
      return i;
    }

    if ((i = cmp(ExtractPlateNumberCode_Samples, notes0, notes1)) != 0) {
      return i;
    }

    if ((i = cmp(MarkerCode_Seq, notes0, notes1)) != 0) {
