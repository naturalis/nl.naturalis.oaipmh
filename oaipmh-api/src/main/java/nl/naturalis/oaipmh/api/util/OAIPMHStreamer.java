...
public class OAIPMHStreamer {
	...
	/**
	 * Expands the {@code xsi:schemaLocation} attribute of the &lt;OAI-PMH&gt; root
	 * element with the specified namespace-location pair. The schema location of
	 * the XSD for OAI-PMH is automatically added. The XSD location for oai_dc,
	 * however, is not automatically added since you might be serving a request for
	 * a different metadata format. However, you can simply add the XSD location for
	 * oai_dc by calling {@link #addOaiDcSchemaLocation()}.
	 * 
	 * @param xmlNamespace
	 *            The XML namespace
